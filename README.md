...
- **`-f <file>`, `--filelist <file>`**   
  Use a plain text `<file>` to define exactly which images should be included in the process. Each line must contain the full path to an image. This option is ideal when you need total control over which images are used - especially if they come from multiple locations.

>[!NOTE]
>Only valid, existing paths ending in `.jpg` or `.jpeg` will be processed. Empty lines and comments are ignored.
